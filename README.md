...
## REMOVE LIST
Данный файл содержит список пакеты, которые будут удалены из текущей системы.

## INSTALL LIST
Данный файл содержит общий список пакетов, которые должны быть установлены **на всех машинах**. Список можно обновить через `pkgsync`, а также Вы должны позаботиться о синхронизации этого списка между всеми Вашими машинами.

## PRESTART SCRIPT
Ваш cкрипт, который запускается **перед** обновлением, удаление пакетов.

## FINISH SCRIPT
Ваш скрипт, который запускается **после** обновления, удаления пакетов.
