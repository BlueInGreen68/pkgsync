...
# Как работает
Ключевой момент работы скрипта основан на команде `comm`. Поочерёдно сравниваются разные списки пакетов и от их вывода будет зависить итоговый результат. [Подробнее](https://linux-faq.ru/page/komanda-comm) о аргументах команды `comm` использованных в скрипте `pkgsync`.
...
Важно! Если машин больше 2, то перед внесением изменений в общий установочный список или список пакетов на удаление, то нужно синхронизировать текущее состояние репозитория на все машины и внести изменения, только затем отправлять новые.
