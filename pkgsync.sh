...
[[ -e "/path/to/config" ]] && . "/path/to/config"
...
if [[ -s "$TMP_DIR/pkg_toinstall.list" ]]; then
    yn=l
    while [[ ! "$yn" =~ ^[YyNnAaQqBb]$ ]]; do
...
## Удаление пакетов
if [[ -s "$TMP_DIR/pkg_toremove.list" ]]; then
    yn=l
    while [[ ! "$yn" =~ ^[YyNnAaQqBb]$ ]]; do
...
if [[ -s "$TMP_DIR/pkg_ourinstall.list" ]]; then
    yn=l
    while [[ ! "$yn" =~ ^[YyNnAaQqEeDd]$ ]]; do
...
if [[ -s "/path/to/pkg_remove.list" ]]; then
    yn=l
    while [[ ! "$yn" =~ ^[YyNnAaQqEe]$ ]]; do
        read -p "Clear pkg_remove.list? (yes/no/list/abort/exclude)..." -n 1 yn
        echo
        [[ "$yn" =~ ^[Ll]$ ]] && cat "/path/to/pkg_remove.list"
    done
    [[ "$yn" =~ ^[Yy]$ ]] && truncate -s 0 "/path/to/pkg_remove.list"
    [[ "$yn" =~ ^[Ee]$ ]] && cat "/path/to/pkg_remove.list" >>"$EXCLUSION_LIST"
    [[ "$yn" =~ ^[AaQq]$ ]] && exit 1
